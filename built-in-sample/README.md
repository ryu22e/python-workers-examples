...
FastAPIを使って、以下のAPIを実装している。

1. LangChainを使って、AIに俳句を詠ませるAPI
2. micropipの使用例1(BeautifulSoup4でPython Boot Campのイベント情報を抽出する）
3. micropipの使用例2(pandasをインストールし、エラーを発生させる）

なお、2024年8月31日時点では、built-in packageは本番環境へのデプロイができない。
